#!/usr/bin/env python3
"""
Rename snake_case function/method names to camelCase (lowerCamelCase).
Applies word-boundary regex replacements across all .cpp, .hpp, .h files.
"""
...
# Complete mapping of old_name -> new_name (all ~200+ renames)
RENAME_MAP = {
    # pj_base/types.hpp
    ...
    # column_buffer.hpp / column_buffer.cpp
    ...
    # topic_storage.hpp / topic_storage.cpp
    ...
    # arrow_import.hpp / arrow_import.cpp
    'schema_from_ipc': 'schemaFromIpc',
    'import_ipc_stream': 'importIpcStream',

    # derived_engine.hpp / derived_engine.cpp
    ...
    # builtin_transforms.hpp / builtin_transforms.cpp (override of ISISOTransform)
    # output_kind already listed above
...
def rename_in_file(filepath):
    """Apply renames to a single file. Returns True if changed."""
    try:
    ...
    # Apply each rename using word boundary matching
...
def main():
    """Scan all .cpp, .hpp, .h files and apply renames."""
