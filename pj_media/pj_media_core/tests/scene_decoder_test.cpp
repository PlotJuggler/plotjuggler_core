...
std::vector<uint8_t> encodeColor(double r, double g, double b, double a) {
  std::vector<uint8_t> body;
  pb::appendTag(body, 1, 1); pb::appendDouble(body, r);
  pb::appendTag(body, 2, 1); pb::appendDouble(body, g);
  pb::appendTag(body, 3, 1); pb::appendDouble(body, b);
  pb::appendTag(body, 4, 1); pb::appendDouble(body, a);
  return body;
}

// Build a Foxglove CircleAnnotation: {2: position, 3: diameter, 4: thickness,
// 5: fill_color, 6: outline_color}
std::vector<uint8_t> encodeCircleAnnotation(double x, double y, double diameter, double thickness,
                                            const std::vector<uint8_t>& fill,
                                            const std::vector<uint8_t>& outline) {
  std::vector<uint8_t> body;
  pb::appendTag(body, 2, 2); pb::appendLenDelim(body, encodePoint2(x, y));
  pb::appendTag(body, 3, 1); pb::appendDouble(body, diameter);
  pb::appendTag(body, 4, 1); pb::appendDouble(body, thickness);
  pb::appendTag(body, 5, 2); pb::appendLenDelim(body, fill);
  pb::appendTag(body, 6, 2); pb::appendLenDelim(body, outline);
  return body;
}
