...
#include "pj_media_core/scene_decoder.h"  // existing reader, used for round-trips
...
// -----------------------------------------------------------------------------
// Hand-rolled Protobuf helpers — same style as the existing decoder test
// (`pj_media/pj_media_core/tests/scene_decoder_test.cpp` lines 136-164).
// Used to build expected byte sequences for golden-byte tests.
// -----------------------------------------------------------------------------
namespace pb {
...
::testing::AssertionResult ColorEq(const ColorRGBA& a, const ColorRGBA& b) {
  ...
  return ::testing::AssertionFailure() << "Color mismatch: got {" << +b.r << "," << +b.g << ","
                                       << +b.b << "," << +b.a << "}, expected {" << +a.r << ","
                                       << +a.g << "," << +a.b << "," << +a.a << "}";
}
...
TEST(ImageAnnotationCodecTest, RoundTrip_AllTopologies) {
  for (auto topology : {AnnotationTopology::kPoints, AnnotationTopology::kLineList,
                        AnnotationTopology::kLineStrip, AnnotationTopology::kLineLoop}) {
    ImageAnnotation in;
    PointsAnnotation pa;
