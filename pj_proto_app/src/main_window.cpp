...
MainWindow::MainWindow(const std::string& plugin_dir, QWidget* parent)
  ...
  if (td_result) default_td_id_ = *td_result;
...
void MainWindow::loadFile(const QString& file_path) {
  ...
  auto session =
      std::make_unique<DataSourceSession>(engine_, source->library, default_td_id_, display_name, this);
...
void MainWindow::onLoadFile() {
  ...
  if (sources.size() > 1) {
    QStringList names;
    for (auto* s : sources) names << QString::fromStdString(s->name);
    bool ok = false;
  ...
  {
    auto base = saved_config.empty()
                    ? nlohmann::json::object()
                    : nlohmann::json::parse(saved_config, nullptr, false);
  ...
  if ((source->capabilities & PJ_DATA_SOURCE_CAPABILITY_HAS_DIALOG) != 0) {
    ...
    if (vt_result) {
      ...
      if (dialog_ctx != nullptr) {
        ...
        if (dialog_engine.showDialog(this) == PJ::DialogResult::kRejected) return;
...
void MainWindow::onStartStream() {
  ...
  auto session =
      std::make_unique<DataSourceSession>(engine_, source->library, default_td_id_, source->name, this);
...
void MainWindow::onClearPlots() { chart_panel_->clearAllSeries(); }
