...
TEST(ObjectIngestPolicyResolverTest, DefaultPolicyIsLazyScalars) {
  ...
  EXPECT_EQ(r.resolve("any_source", "/any/topic", CanonicalObjectKind::kImage),
...
TEST(ObjectIngestPolicyResolverTest, KindOverrideFiresOnMatch) {
  ...
  EXPECT_EQ(r.resolve("src", "/lidar/points", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kPureLazy);
  // Different kind falls through to default.
  EXPECT_EQ(r.resolve("src", "/cam/image", CanonicalObjectKind::kImage),
            ObjectIngestPolicy::kLazyObjectsEagerScalars);
}
...
TEST(ObjectIngestPolicyResolverTest, SourceOverridesKind) {
  ...
  EXPECT_EQ(r.resolve("mcap_source", "/lidar/points", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kEager);
  // Different source → kind override fires.
  EXPECT_EQ(r.resolve("ros2_stream", "/lidar/points", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kPureLazy);
}
...
TEST(ObjectIngestPolicyResolverTest, TopicOverridesEverything) {
  ...
  EXPECT_EQ(r.resolve("mcap_source", "/diagnostics/lidar", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kPureLazy);
  // Different topic → source override fires.
  EXPECT_EQ(r.resolve("mcap_source", "/other/lidar", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kEager);
}
...
TEST(ObjectIngestPolicyResolverTest, TypicalApplicationSetup) {
  ...
  EXPECT_EQ(r.resolve("mcap", "/cam/raw", CanonicalObjectKind::kImage),
            ObjectIngestPolicy::kLazyObjectsEagerScalars);
  EXPECT_EQ(r.resolve("mcap", "/cam/jpeg", CanonicalObjectKind::kCompressedImage),
            ObjectIngestPolicy::kPureLazy);
  EXPECT_EQ(r.resolve("mcap", "/lidar", CanonicalObjectKind::kPointCloud),
            ObjectIngestPolicy::kPureLazy);
  // Scalar-only topic (no canonical) takes the default.
  EXPECT_EQ(r.resolve("mcap", "/diagnostics", CanonicalObjectKind::kNone),
            ObjectIngestPolicy::kLazyObjectsEagerScalars);
}
...
TEST(ObjectIngestPolicyResolverTest, LastWriteWinsForSameKey) {
  ...
  EXPECT_EQ(r.resolve("src", "/topic", CanonicalObjectKind::kImage),
            ObjectIngestPolicy::kPureLazy);
