...
- **G6 — Engine-first ingestion.** The canonical storage target is the `pj_datastore` engine (`DataEngine`, chunked storage, typed writer/reader). Legacy structures are transitional compatibility views.
...
From the `pj_base` source, there are 6 plugin categories:
...
This section binds plugin architecture to the current `pj_datastore` engine state.
...
- Source of truth for newly migrated paths is the `pj_datastore` engine.
...
The dialog protocol is **fully implemented** at `pj_plugins/dialog_protocol/`. It provides a C ABI vtable for plugins to describe UI layout (`.ui` XML) and communicate with the host through JSON data binding.

## 8.1 Implementation Files

| Path | Purpose |
|---|---|
| `pj_plugins/dialog_protocol/include/pj_plugins/dialog_protocol.h` | C ABI vtable (`PJ_dialog_vtable_t`) |
| `pj_plugins/dialog_protocol/include/pj_plugins/host/` | Host-side C++ API (`PJ` namespace) |
| `pj_plugins/dialog_protocol/include/pj_plugins/host_qt/` | Qt dialog engine (`PJ` namespace) |
| `pj_plugins/dialog_protocol/include/pj_plugins/sdk/` | Plugin-side SDK helpers |
| `pj_plugins/dialog_protocol/src/dialog_engine.cpp` | Qt dialog engine implementation |
| `pj_plugins/dialog_protocol/src/widget_binding.cpp` | Widget binding implementation |
| `pj_plugins/dialog_protocol/tests/` | Protocol and integration tests |
| `pj_plugins/dialog_protocol/examples/` | Example plugin using the protocol |
...
1. Resolve toolchain/build compatibility for `pj_datastore/` integration path.
...
1. Host-rendered dialog protocol opt-in (`.ui + JSON` via `pj_plugins/dialog_protocol/`).
