...
TEST(ChunkTest, IntegerTypesRoundTrip) {
  ...
  std::vector<ColumnDescriptor> cols = {
      ...
      make_col(3, PrimitiveType::kInt32, "i32"),
      make_col(4, PrimitiveType::kInt64, "i64"),
      make_col(5, PrimitiveType::kUint8, "u8"),
      make_col(6, PrimitiveType::kUint16, "u16"),
      make_col(7, PrimitiveType::kUint32, "u32"),
      make_col(8, PrimitiveType::kUint64, "u64"),
  };
...
TEST(ChunkTest, BulkReadFloat32) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/130, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
  ...
  chunk.read_column_as_doubles(0, out.data(), 0, kRows);
  for (int i = 0; i < kRows; ++i) {
    EXPECT_FLOAT_EQ(static_cast<float>(out[static_cast<std::size_t>(i)]),
                    static_cast<float>(i) * 1.5F) << "row " << i;
  }

  // Read a sub-range [3, 7)
  std::vector<double> sub(4);
  chunk.read_column_as_doubles(0, sub.data(), 3, 4);
  for (int i = 0; i < 4; ++i) {
    EXPECT_FLOAT_EQ(static_cast<float>(sub[static_cast<std::size_t>(i)]),
                    static_cast<float>(i + 3) * 1.5F) << "sub row " << i;
  }
}
...
TEST(ChunkTest, BulkReadInt64) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/140, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
  ...
  for (int i = 0; i < kRows; ++i) {
    EXPECT_DOUBLE_EQ(out[static_cast<std::size_t>(i)],
                     static_cast<double>(values[i])) << "row " << i;
  }
}
...
TEST(ChunkTest, BulkReadBoolStringReturnsNaN) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/150, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
  ...
  chunk.read_column_as_doubles(0, out.data(), 0, 2);
  EXPECT_TRUE(std::isnan(out[0]));
  EXPECT_TRUE(std::isnan(out[1]));

  chunk.read_column_as_doubles(1, out.data(), 0, 2);
  EXPECT_TRUE(std::isnan(out[0]));
  EXPECT_TRUE(std::isnan(out[1]));
}
...
TEST(ChunkTest, BulkReadZeroRows) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/160, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
  ...
  double dummy = 0.0;
  chunk.read_column_as_doubles(0, &dummy, 0, 0);
...
TEST(ChunkTest, ConstantIntColumnGetsConstantEncoding) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/200, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  chunk.read_column_as_doubles(0, out.data(), 0, 100);
  for (std::size_t i = 0; i < 100; ++i) {
...
TEST(ChunkTest, ConstantFloatColumnGetsConstantEncoding) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/201, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
...
TEST(ChunkTest, NarrowRangeIntColumnGetsFOR) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/202, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  const auto& for_enc = std::get<encoding::FrameOfReferenceEncoded>(
      chunk.encoding_data[0]);
  ...
  for (std::size_t i = 0; i < 101; ++i) {
    EXPECT_DOUBLE_EQ(chunk.read_numeric_as_double(0, i),
                     1000.0 + static_cast<double>(i)) << "row " << i;
  }

  // Bulk read
  std::vector<double> out(101);
  chunk.read_column_as_doubles(0, out.data(), 0, 101);
  for (std::size_t i = 0; i < 101; ++i) {
    EXPECT_DOUBLE_EQ(out[i], 1000.0 + static_cast<double>(i))
        << "bulk row " << i;
  }
}
...
TEST(ChunkTest, WideRangeIntColumnStaysRaw) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/203, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
...
TEST(ChunkTest, FloatColumnAlwaysStaysRaw) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/204, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
...
TEST(ChunkTest, ConstantBoolGetsConstantEncoding) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/205, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
...
TEST(ChunkTest, BulkAppendFloat32) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/500, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  builder.append_timestamps(ts.data(), N);
  builder.append_column_float32(0, x_vals.data(), N);
  builder.append_column_float32(1, y_vals.data(), N);
  ...
  for (std::size_t i = 0; i < N; ++i) {
    ...
    EXPECT_FLOAT_EQ(static_cast<float>(chunk.read_numeric_as_double(0, i)),
                    x_vals[i]);
    EXPECT_FLOAT_EQ(static_cast<float>(chunk.read_numeric_as_double(1, i)),
                    y_vals[i]);
  }
}
...
TEST(ChunkTest, BulkAppendStats) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/501, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);

  const double data[] = {3.0, 1.0, 4.0, 1.0, 5.0};
  const Timestamp ts[] = {10, 20, 30, 40, 50};

  builder.append_timestamps(ts, 5);
  builder.append_column_float64(0, data, 5);
  builder.finish_bulk_append();
...
TEST(ChunkTest, BulkAppendConstantColumn) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/502, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  builder.append_timestamps(ts.data(), N);
  builder.append_column_int32(0, vals.data(), N);
  builder.finish_bulk_append();
...
TEST(ChunkTest, BulkAppendStrings) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/503, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  builder.append_timestamps(ts, 3);
  builder.append_column_strings(0, offsets, string_data, 3);
  builder.finish_bulk_append();
...
TEST(ChunkTest, BulkRemainingCapacity) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/504, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
  ...
  builder.append_timestamps(ts, 3);
  builder.append_column_float32(0, vals, 3);
  builder.finish_bulk_append();
...
TEST(ChunkTest, BulkAppendWithValidity) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/505, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);

  const double data[] = {1.0, 0.0, 3.0, 0.0};
  const Timestamp ts[] = {10, 20, 30, 40};

  builder.append_timestamps(ts, 4);
  builder.append_column_float64(0, data, 4);
  // validity: bits [1, 0, 1, 0] = 0b0101 = 0x05
  const uint8_t bitmap[] = {0x05};
  builder.append_column_validity(0, bitmap, 0, 4);
  builder.finish_bulk_append();
...
TEST(ChunkTest, BulkAppendMixedTypes) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/506, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/1000);
  ...
  builder.append_timestamps(ts, 3);
  builder.append_column_float32(0, f32, 3);
  builder.append_column_int64(1, i64, 3);
  builder.append_column_bool(2, bools, 3);
  builder.finish_bulk_append();
...
#define PJ_EXPECT_ASSERT_FAIL(stmt, msg) \
  EXPECT_THROW(stmt, std::runtime_error)
#else
#define PJ_EXPECT_ASSERT_FAIL(stmt, msg) \
  ASSERT_DEATH(stmt, msg)
#endif
...
TEST(ChunkDeathTest, SetWithoutBeginRowAsserts) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/300, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
...
TEST(ChunkDeathTest, FinishRowWithoutBeginRowAsserts) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/301, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
...
TEST(ChunkDeathTest, BeginRowWhileRowInProgressAsserts) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/302, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
...
TEST(ChunkDeathTest, OutOfBoundsColIndexAsserts) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/303, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
...
TEST(ChunkDeathTest, OutOfOrderTimestampAsserts) {
  ...
  TopicChunkBuilder builder(/*topic_id=*/304, /*schema_id=*/1,
                            std::move(cols), /*max_rows=*/100);
