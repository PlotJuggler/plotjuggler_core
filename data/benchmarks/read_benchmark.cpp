  ...
  switch (chunk.column_encodings[col]) {
    case EncodingType::kConstant: {
      const auto& enc = std::get<encoding::ConstantEncoded>(
          chunk.encoding_data[col]);
      return static_cast<double>(enc.value.size()) / chunk.stats.row_count;
    }
    case EncodingType::kFrameOfReference: {
      const auto& enc = std::get<encoding::FrameOfReferenceEncoded>(
          chunk.encoding_data[col]);
    ...
    case EncodingType::kDictionary: {
      ...
      return static_cast<double>(dict.indices.size() + dict_bytes) /
             chunk.stats.row_count;
    }
    default:
      return static_cast<double>(chunk.encoded_columns[col].size()) /
             chunk.stats.row_count;
  }
}
...
void BM_Chunk_ReadFloat32(benchmark::State& state) {
  static TopicChunk chunk = build_typed_chunk(
      ...
      [](TopicChunkBuilder& b, std::size_t col, int i) {
        b.set_float32(col, static_cast<float>(i) * 0.1f);
      });
...
void BM_Chunk_BulkReadFloat32(benchmark::State& state) {
  ...
  for (auto _ : state) {
    chunk.read_column_as_doubles(0, buf.data(), 0, kPointCount);
...
void BM_Cursor_ReadFloat32(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    cursor.for_each([&](const SampleRow& row) {
      sum += row.chunk->read_numeric_as_double(0, row.row_index);
    });
    benchmark::DoNotOptimize(sum);
    benchmark::ClobberMemory();
  }
...
void BM_Cursor_ReadString(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    auto cursor = range_query(chunks, 0, kPointCount - 1);
    cursor.for_each([&](const SampleRow& row) {
      total_len += row.chunk->read_string(0, row.row_index).size();
    });
    benchmark::DoNotOptimize(total_len);
    benchmark::ClobberMemory();
  }
...
void BM_Cursor_ChunkAtATime_Float32(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    cursor.for_each_chunk([&](const ChunkRowRange& range) {
      ...
      range.chunk->read_column_as_doubles(0, buf.data(), range.row_start, n);
...
void BM_Deque_ReadString(benchmark::State& state) {
  ...
  static std::deque<std::pair<Timestamp, std::string>> data =
      BuildDequeData<std::string>([](int i) {
        return kEnumValues[static_cast<std::size_t>(i) % kEnumValues.size()];
      });
