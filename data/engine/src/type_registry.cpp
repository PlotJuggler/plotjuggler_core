...
void flatten_leaf_types_impl(
    const TypeTreeNode& node, std::string_view prefix, std::vector<std::pair<std::string, PrimitiveType>>& out) {
...
std::vector<std::pair<std::string, PrimitiveType>> flatten_leaf_types(const TypeTreeNode& root) {
...
absl::StatusOr<SchemaId> TypeRegistry::register_schema(
  ...
  if (name_to_id_.contains(schema_name)) {
    return absl::AlreadyExistsError(absl::StrCat("Schema '", schema_name, "' already registered"));
  }
...
std::optional<SchemaId> TypeRegistry::find_by_name(std::string_view name) const {
...
absl::Status TypeRegistry::evolve_schema(SchemaId id, std::shared_ptr<TypeTreeNode> updated_tree) {
  auto it = schemas_.find(id);
  if (it == schemas_.end()) {
    return absl::NotFoundError(absl::StrCat("Schema ID ", id, " not found"));
  }
  ...
  for (const auto& [old_path, old_type] : old_leaves) {
    ...
    if (new_it == new_leaf_map.end()) {
      return absl::InvalidArgumentError(absl::StrCat("Field '", old_path, "' was removed in the updated schema"));
    }
    if (new_it->second != old_type) {
      return absl::InvalidArgumentError(absl::StrCat("Field '", old_path, "' changed type in the updated schema"));
    }
  }
