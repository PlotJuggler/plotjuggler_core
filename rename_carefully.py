#!/usr/bin/env python3
"""
Carefully rename functions using precise regex patterns that avoid struct members, parameters, etc.
Process module by module with verification.
"""
...
def rename_module(module_name: str, renames: dict):
    """Rename functions in a module."""
    print(f"\n{'='*60}")
    ...
    for old_name, new_name in sorted(renames.items()):
        ...
        cmd = ['grep', '-l', old_name, '-r', 'pj_base', 'pj_datastore', 'pj_plugins']
        ...
        if result.stdout:
            for filepath_str in result.stdout.strip().split('\n'):
                ...
                # Rename function definitions: word boundaries
                # This matches:  "  functionName(" or "::functionName(" or "} functionName(" etc.
                content = re.sub(
    ...
    print(f"Files updated: {total_renames}")
...
def main():
    ...
    for module_name, renames in RENAMES_BY_MODULE.items():
        count = rename_module(module_name, renames)
        grand_total += count

    print(f"\n{'='*60}")
    print(f"GRAND TOTAL: {grand_total} files updated")
    print(f"{'='*60}")
